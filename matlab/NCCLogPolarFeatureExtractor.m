classdef NCCLogPolarFeatureExtractor < helpers.GenericInstaller ...
...
% AUTORIGHTS
  ...
  methods
    function obj = NCCLogPolarFeatureExtractor(varargin)
...
%     end
    ...
    function [frames descriptors] = extractDescriptors(obj, imagePath, frames)
      ...
      vectorOptionNCCBlock = org.opencv.contrib.Contrib.extract(extractor, imageMat, matOfKeyPoint);
      
      assert(vectorOptionNCCBlock.getSize == size(frames, 2));
      
      extractionMask = [];
      for index = 1 : size(frames, 2)
        if vectorOptionNCCBlock.isDefinedAtIndex(index - 1)
          extractionMask = [extractionMask 1];
        else
          extractionMask = [extractionMask 0];
        end
      end
      
      % Remove the unextractable descriptors and the corresponding frames.
      frames = frames(:, boolean(extractionMask));
      vectorNCCBlock = org.opencv.contrib.Contrib.flatten(vectorOptionNCCBlock);
      
      assert(size(frames, 2) == vectorNCCBlock.getSize);
      
      descriptors = [];
      for index = 1 : size(frames, 2)
        entry = vectorNCCBlock.getAtIndex(index - 1);
        descriptors = [descriptors entry];
      end
      
      assert(length(descriptors) == size(frames, 2));
      
%       descriptors = vectorNCCBlock;
